...
class utility
{
    ...
    public static function get_course_image($core_course_list_element, $islist = false) {
        ...
        if (!empty($courseimage)) {
            ...
            return $OUTPUT->get_generated_image_for_id($core_course_list_element->id);
        }
    }
            ...
            foreach ($sections_data[$mod] as $activity_id) {
                ...
                if (!in_array($activity->modname, $excluded_mods)) {
                    ...
                    $sections[$count]['activity_list'][] = array(
                        'active' => $active,
                        'name' => strip_tags(format_text($activity->name)),
  ...
  // send customer feedback to RemUI team.
    ...
    public static function get_recent_accessed_courses($limit) {
        ...
        $courses = $DB->get_records($table, array('userid' => $USER->id), 'timeaccess desc', 'id', 0, $limit);
        ...
        $fincourses = $DB->get_records_sql($sql, $inparams, 0, 0);
    ...
    public static function get_course_cards_content($wdmdata, $date = 'all') {
        ...
        if ($wdmdata->pagination) {
            ...
            $totalcourses  = self::get_courses(true, $search, $category, $startfrom, $limitto, $mycourses, $categorysort, $courses);
        ...
        // Fetch the courses
        $courses = self::get_courses(false, $search, $category, $startfrom, $limitto, $mycourses, $categorysort, $courses);
